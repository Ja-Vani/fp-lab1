...
%fold
is_spicial({A, B, C, K}, {}) when A + B + C == K ->
    {A, B, C};
is_spicial(_, Acc) ->
    Acc.

get_fold_special_list(K) ->
    get_fold_special_list(1, 1, math:pow(2, 0.5), K).

get_fold_special_list(A, B, C, K) when B < K / 2 ->
    [{A, B, C, K} | get_fold_special_list(A,
                                          B + 1,
                                          math:pow(A * A + (B + 1) * (B + 1), 0.5),
                                          K)];
get_fold_special_list(A, B, C, K) when A < K / 2 ->
    [{A, B, C, K} | get_fold_special_list(A + 1,
                                          A + 1,
                                          math:pow(2 * (A + 1) * (A + 1), 0.5),
                                          K)];
